...
async def get_player_info(
    ...
    properties: Optional[List[str]] = Query(
        None, description="Filter specific properties"
    ),
):
    ...
    try:
        ...
        raise HTTPException(
            status_code=404, detail="Player not found in player details"
        )
    ...
    try:
        ...
        logger.warning(
            f"Name information not found for player {player_id} in wagon {wagon_id}"
        )
        name_info = {}
    ...
    logger.info(
        f"Successfully retrieved player info for {player_id} in wagon {wagon_id}"
    )
    return result
